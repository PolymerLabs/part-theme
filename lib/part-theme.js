...
function propsForPart(name, element, requireTheme) {
  const host = element && element.getRootNode().host;
  if (!host) {
    return;
  }
  // collect props from host element.
  let props = propsFromElement(name, host, requireTheme);
  // now recurse ancestors to find matching `theme` properties
  const themeProps = propsForPart(name, host, true);
  ...
  if (!requireTheme) {
    ...
    partInfo.forEach(info => {
      ...
      if (name == info.forward || catchAll) {
        ...
        const forwarded = propsForPart(ancestorName, host);
